		...
		<form method="post" id="projectTypeForm">
			<table class="form">
				...
				<tr>
					...
					<td><input type="text" value="${projectType.title}" id="title" class="text" name="title" /></td>
					<th><label for="sortOrder">Order Number</label></th>
					<td><input type="text" value="${projectType.sortOrder}" id="sortOrder" class="text" name="sortOrder" /></td>
				</tr>
				<tr>
					<th><label for="remark">Remark</label></th>
					<td><textarea id="remark" class="area" name="remark">${projectType.remark}</textarea></td>
				</tr>
			</table>
			<div class="controls">
        ...
        $("#projectTypeForm").validate({
			rules: {
				title: {
					required: true,
					remote: {
						url : "<c:url value='/projectType/checkDuplicate' />",
						data : {
							title : function(){
								return $("#title").val()
							},
