...
<div class="request">
	...
	<div class="btn-group">
		<div class="left">
			<button type="button" id="btnAskUnSubmit" class="btn white">Request Unsubmit</button>
			<button type="button" id="btnSubmitChecked" class="btn white">Submit Checked</button>
		</div>
		<div class="right">
			<button type="button" id="btnSubmitAll" class="btn white">Submit All</button>
			<button type="button" id="btnSave" class="btn white">Save</button>
		</div>
	</div>
</div>
<div id="dlgUnsubmit">
	...
	var timecard = {
			...
			bindingEvent : function(){
				...
				$("#btnSave").on("click", function(){
					...
					timecard.grid.jqGrid('saveRow', timecard.lastsel);
					timecard.lastsel = null;
					
					if(!timecard.isValidTimecard()){
						Yuga.dialog.alert("Timecard is bigger than 24 hours.");
						return;
					}
					
			    	timecard.saveAll(CONSTANT.TIMECARD_STATUS_SAVE);
				});
				...
				$("#btnSubmitAll").on("click", function(){
			    	...
			    	timecard.saveAll(CONSTANT.TIMECARD_STATUS_SUBMIT);
				});
				
				$("#btnSubmitChecked").on("click", function(){
					...
					timecard.submitChecked();
				});
				
				$("#btnAskUnSubmit").on("click", function(){
					...
					dlg.dialog({
						modal : true,
						width : 800,
						title : "Unsubmit Request",
						open  : function(){
							
							$("#submitList").trigger("reloadGrid");
							$("#submitList").jqGrid({

								url 		: "<c:url value='/timecard/getSubmitTimecardGrid'/>",
								colNames 	: ['ID', 'Project ID', 'Project Name', 'Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'],
								rownumbers	: true,
								multiselect	: true,
								sortable: false,
								...
								colModel 	: [
							   		{ name	:'timecardProjectID', index	:'timecardProjectID', hidden	: true },
							   		{ name	:'projectID', index	:'projectID', hidden	: true},
							   		{ name	:'project', index	:'project', width	: 200,
							   			formatter : function(cellvalue, options, rowObject){
							   				return cellvalue.projectName;
							   			}
							   		},
							   		{ name	:'day1', index	:'day1', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day2', index	:'day2', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day3', index	:'day3', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day4', index	:'day4', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day5', index	:'day5', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day6', index	:'day6', width	: 50, align	: 'center', formatter : removeZeroFormatter },
							   		{ name	:'day7', index	:'day7', width	: 50, align	: 'center', formatter : removeZeroFormatter }
							   	],
							});
						},
						buttons : {
							"Request Checked" : function(){
								...
								var rows = $("#submitList").getGridParam("selarrrow");
								if (rows.length) {
							    	
									var timcard 	= { timecardProjectList : [] };
									
							    	for(var i = 0; i < rows.length; i ++){
					    				var timecardProjectID 	= $("#submitList").jqGrid("getCell", rows[i], "timecardProjectID");
										var projectID 			= $("#submitList").jqGrid("getCell", rows[i], "projectID");
										
										if(timecardProjectID != ""){
									...
									var url = "<c:url value='/timecard/requestUnsubmit' />";
									...
									$.ajax({
										...
										success : function(response){
											...
											Yuga.dialog.alert("Request has been sent.");
										}
									});
							      }
			...
			saveAll : function(status){
				...
				var startDate 	= timecard.date.getStartDate(timecard.dateFormat),
				...
				timecard.grid.find("tr").each(function(){
					...
					var rowId = $(this).attr("id"),
						projectID = timecard.grid.jqGrid("getCell", rowId, "projectID");
					
					if(projectID != false){

	    				var day1 				= timecard.grid.jqGrid("getCell", rowId, "day1") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day1") ;
	    				var day2 				= timecard.grid.jqGrid("getCell", rowId, "day2") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day2") ;
	    				var day3 				= timecard.grid.jqGrid("getCell", rowId, "day3") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day3") ;
	    				var day4 				= timecard.grid.jqGrid("getCell", rowId, "day4") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day4") ;
	    				var day5 				= timecard.grid.jqGrid("getCell", rowId, "day5") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day5") ;
	    				var day6 				= timecard.grid.jqGrid("getCell", rowId, "day6") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day6") ;
	    				var day7 				= timecard.grid.jqGrid("getCell", rowId, "day7") == '' ? 0 : timecard.grid.jqGrid("getCell", rowId, "day7") ;
	    				var timecardProjectID 	= timecard.grid.jqGrid("getCell", rowId, "timecardProjectID");

	                    var weekMM 				= timecard.grid.jqGrid("getCell", rowId, "weekMM").toNumber();
	                    var currentMM 			= timecard.grid.jqGrid("getCell", rowId, "currentMM").toNumber();
	                    var assignedMM 			= timecard.grid.jqGrid("getCell", rowId, "assignedMM").toNumber();
	                    var totalHour 			= timecard.grid.jqGrid("getCell", rowId, "totalHour").toNumber();
	    				...
	    				timcard.timecardID = timecard.grid.jqGrid("getCell", rowId, "timecardID");
					}
				});
				...
				if(timcard.timecardProjectList.length > 0){
			...
			moveDate : {
				...
				refreshNavigator : function(){
				 	var maxDate = timecard.dpWeek.datepicker('getDate').addDays(7);
					var minDate = (new Date()).addDays(-1).saturday();
					
					
					// if last day of date picker > currentDate
			...
			applyFilter : function(){
				...
				var currentDate = timecard.dpWeek.datepicker('getDate').addDays(-7).sunday(),
					...
					options = {
						url 		: "<c:url value='/timecard/getGrid'/>",
									...
									if(rowObject.status == CONSTANT.TIMECARD_STATUS_SUBMIT){
										$div.addClass("ico yes");
						   				...
						   				if(totalMM > rowObject.plannedWorkSize && 
						   					...
						   					// Restrict project manager can not request overtime of own project
						...
						loadComplete : function(data){
						    ...
						    timecard.moveDate.refreshNavigator();
						}, 
