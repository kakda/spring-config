...
<script type="text/javascript">
	...
	var memberTab = {
			...
			generateHtml : function(dropElement,e, type){
				...
				newHtml.attr( "plannedMemberID",  dropElement.attr('plannedMemberID') );
			...
			bindEvent: function( tree ){
				$('.mmlist').on('click','.btnRemove, .remove, .minus',function(){
					var nodeData ="";
					if($(this).hasClass('groupRM')){
						...
						var newProjectMemberId = parent.attr('plannedmemberid'); 
						...
						if(newProjectMemberId !== undefined){
							for(var removedMember in removedMembers){
								if(newProjectMemberId === removedMembers[removedMember].plannedmemberid){
...
/*  					var groupMemberHtml = "<tr><td colspan='6'><div class='inpMember'><input type='text' value='0' class='text planmm' />";
				...
				$("#btnUpdate").click(function(){
					...
					if( isValid ){
						...
						var addMembers = [];
						var updateMembers = [];
						var url = '<c:url value="/project/"/>' + page.projectID + "/planned/members";
...
//												position: $( data ).attr( "position" ),
												...
												plannedMemberGroupID : groupId,
												plannedMemberID :  $( data ).attr( "plannedmemberid" )
										}
										if($( data ).attr( "groupid" ) != null || $( data ).attr( "plannedmemberid" ) != null){
...
//												position: $( data ).attr( "position" ),
								...
								// is Member
	 							var member = {
									...
									plannedMemberID : $( element ).attr( "plannedMemberID" ),
...
//									position : $( element ).attr( "position" ),
								...
								if(typeof member.plannedMemberID === "undefined"){
...
//    			   					cache			: false,
						...
						var newUrl = "<c:url value='/project/update/' />" + CONSTANT.PROJECT_STATUS_PROJECT_MEMBER;
					...
					if($(this).hasClass('groupRM')){
 					...
 					if( actualMM <= 0 || actualMM == "") {
 						memberTab.removedOldMembers.push({
 							plannedMemberID : typeof tr.attr("plannedmemberid") !== "undefined" ? tr.attr("plannedmemberid") : "",
 							employeeID : typeof tr.attr("employeeid") !== "undefined" ? tr.attr("employeeid") : tr.attr("groupid"),
 							type : type,
 							plannedMemberGroupID : typeof tr.attr("groupid") !== "undefined" ? tr.attr("groupid") : "",
						});
						tr.remove();
					}else{
...
/*   							newHtml = $("<tr employeeid='"+data.employeeID+"' projectmemberid='"+data.employeeID+"'><td colspan='6'><div class='inpMember'><input type='text' value='"+data.plannedWorkSize+"' class='text planmm' /><label class='actualMM'>"+data.actualWorkSize.toFixed(2)+"</label><button type='button' class='btnRemove groupRM remove btn red'>X</button></div><fieldset class='dropHere'><legend><input type='text' id='"+data.employeeID+"'name='groupName' value='"+data.groupName+"'class='groupName'/></legend><div class='scroll'><table class='table tbl01'><tbody>");
							...
							$.each(data.member,function(index, groupemp){
								...
								groupMMhtml.attr( "plannedMemberID", groupemp.plannedMemberID);
							...
							newHtml.attr( "plannedMemberID", data.plannedMemberID  );
							if( data.type == "TBN" ){
								newHtml.attr("TBN", "TBN");
								newHtml.addClass( "drop" );
							}
							...
							newHtml.append( "<td>" + (data.org != undefined ?  data.name : data.name.substring(0,lengthName-1) + "<span>"+data.name.substring(lengthName-1 ,lengthName)+"</span>")+ "</td>" );
		...
		appendRow : function(nodeDetail, tree, targetElement){
			...
			if( !isExist ){
				...
				if( $(targetElement).hasClass( "dropHere" )){
					...
					var plannedMemberID = $( targetElement).parent().attr( "plannedMemberID" );
					newHtml.attr( "plannedMemberID", plannedMemberID  );
					
					// Keep old row into object treeUtil.removedMembers before replace with new row.
					treeUtil.removedMembers.push({'plannedmemberid' : plannedMemberID, 'tr' : $( targetElement ).parent()});
